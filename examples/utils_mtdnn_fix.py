...
class MegaDataSet(object):
    ...
    def batchfy(self, features, batch_size):
        l = len(features)
        batch_list = []
        for x in range(0, l, batch_size):
            batch_list.append(features[x:min(x+batch_size, l)])
        return batch_list
    ...
    def load_MTDNN_dataset(self, batch_size, debug=False):
        ...
        for task in self.task_list:
            if debug:
                features,_, task_ids = self.load_single_dataset(task, batch_size, "debug")
            else:
                features, _, _ = self.load_single_dataset(task, batch_size, "train")
